...
This lab is designed as a set of tasks to help customers with configuring (via automation where available or via the GUI where no automation yet exists) Identity Services Engine (ISE) with the appropriate settings to enable testing of various Authentication and Authorization tasks from other Catalyst Center Automation labs in this series. Within the lab, we will use various tools and techniques to Automate various tasks and orchestrate ISE.

## General Information

In this lab we will focus on configuring the integration between ISE and Catalyst Center, and then utilizing REST-API endpoints to implement the Policy Elements/Sets to enable ISE to serve as the Authentication and Authorization mechanism for Catalyst Center deployments.  Note that not *all* tasks can be completed via a REST-API -- at least in the versions of ISE/CatC currently running in the lab -- but more features are being enabled via REST-API every day.  Please check the appropriate API documentation for your own ISE/CatC deployment for the most up-to-date list of API endpoints.
...
This lab is designed to be run in Cisco dClouds - Enterprise Network Sandbox Lab. It was developed in version 4, including Catalyst Center 2.3.5.6, ISE 3.2.0.542 (Patch 5) and equipment running at least 17.x versions of code. [Versions as of July 2025]
